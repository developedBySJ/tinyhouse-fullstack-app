...
exports.bookingResolver = {
    Booking: {
        id: (bookings) => bookings._id,
        listings: (bookings, _args, { db }) => {
            return db.listings.findOne({ _id: bookings.listings });
        },
        tenant: (bookings, _args, { db }) => {
            return db.users.findOne({ _id: bookings.tenant });
        }
    },
    Mutation: {
        createBooking: (_root, { input }, { db, req }) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                ...
                yield db.users.findOneAndUpdate({ _id: host._id }, { $inc: { income: totalPrice } });
                console.log(bookingsIndex);
                yield db.listings.findOneAndUpdate({ _id: listing._id }, {
